...
\begin{tcolorbox}[sectionbox=Introduction]
  ...
  \hrule
  \vspace{.5cm}
    POSYDON is an open source tool for simulation, interpretation, and analyzation of binary systems. While POSYDON is heavily reliant on MESA grids, it takes a largely different approach. POSYDON takes an initial zero-age main sequence (ZAMS) system and evolves it through a series of grids and solvers, recording the properties at the start and end of each stage. By taking this approach, it allows for systems with vast, complicated evolutionary paths to be recorded from start to end. POSYDON provides the framework to do this process in a high performance computing environment, allowing massive and rapid analysis \parencite{posydon2023}. See table~\ref{tab:POSYDONDataExample} for an example of data structure.
  ...
  \vspace{.5cm}
    ...
    For my reference populations I made the choice to evolve a grid with as minimal of initial biasing. In order to do this, I sourced a finished \(1Z_\odot\) POSYDON grid of a million systems where all the systems were initially binaries, but with random mass ratios, eccentricities, and periods. This allowed for a dataset which contained binaries in all stages of evolution, at a wide range of masses, allowing for more detailed analysis.
...
\begin{tcolorbox}[sectionbox={Methods}, breakable=true]
  ...
  \vspace{.5cm}
    ...
    \begin{center}
      ...
      \captionof{figure}{W Ursae Majoris observed temperature and luminosity in comparison to evolved contact binaries. Note that the CE solver has been altered since this population.}
...
\begin{tcolorbox}[sectionbox=Previous Results and Current Questions]
  ...
  With W UMa and Vela X-1 the results, while on the more extreme side, were expected, however, with V404 Cygni the results seemed to hint at a much larger phenomenon  
